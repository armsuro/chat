...
routes.get('/', (req, res) => {
    res.send({api_version: 1});
});

routes.post('/auth/login', (req, res, next) => {
    checkRequestParamets(req, res, next, ['username', 'password'])
}, UserController.login);

routes.post('/auth/logout', authenticate, UserController.logout);

routes.post('/quote', [authenticate, (req, res, next) => {
    checkRequestParamets(req, res, next, ['name'])
}, checkRoleCustomer], QuoteController.create);

routes.put('/quote/assign/:id', [authenticate, (req, res, next) => {
    checkRequestParamets(req, res, next, ['id'])
}, checkRoleRepairshop], QuoteController.assign);
...
routes.put('/quote/:id', [authenticate, (req, res, next) => {
    checkRequestParamets(req, res, next, ['name', 'id'])
}, checkRoleCustomer], QuoteController.update);

routes.delete('/quote/:id', [authenticate, (req, res, next) => {
    checkRequestParamets(req, res, next, ['id'])
}, checkRoleCustomer], QuoteController.destroy);
